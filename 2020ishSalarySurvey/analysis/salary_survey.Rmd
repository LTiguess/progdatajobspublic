...
Olivia note 4/11 -- this seems to be fixed with my edit to the cat_cross.R function, not sure why yet 
```{r, message = FALSE, echo = FALSE }
cat_cross(x = df, 
  question = seniority_level, 
...
**working on fixing graph**

```{r, message = FALSE, echo = FALSE, fig.width = 14, fig.height = 12 }
# make a df of just the columns/values we are interested in
tmp <- df %>%
...
plot_data <- tmp2 %>%
  ...
  mutate(
    ...
    value = factor(
      value, 
      levels = c("Advanced", "Intermediate", 
                  "Never used this before", "Introductory", "Novice"))
  ) %>%
  # add cleaned version of responses 
  left_join(
    col_names,
...
right <- plot_data %>% filter(value %in% c("Advanced", "Intermediate"))
left <- plot_data %>% filter(value %in% c("Novice", "Introductory", "Never used this before"))

# make plot
p <- ggplot() + 
  geom_col(data = right, aes(x = reorder(response_name, order), y = pct, fill = value)) +
  geom_col(data = left, aes(x = reorder(response_name, order), y = -pct, fill = value), 
...
<!-- # ```{r} -->
<!-- # plot_likert(mydf) + theme_minimal() scale_fill_viridis_d()  -->
<!-- # plot_likert(tmp)+ theme_minimal() -->
<!-- # ``` -->
...
TO DO condense options in free text
...
plot_salary(df, var) + scale_colour_viridis_d(option = "A")
```
...
plot_salary(df, var) +  scale_colour_viridis_d(option = "B")
```
...
### Sexuality 
```{r, message = FALSE, echo = FALSE }
var <- substitute(sexual_orientation)

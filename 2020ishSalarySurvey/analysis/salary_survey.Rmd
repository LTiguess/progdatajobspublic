...
all <- df %>%
  ...
  mutate(gender_bin = "All")

by_demo <- df %>%
  select(num_staffers, gender_bin) %>%
  filter(!is.na(num_staffers), !is.na(gender_bin)) %>%
  bind_rows(all)  %>% 
  # create a weight variable with all respondents have the same weight, so we can use the crossstab function, which requires a weight 
  mutate(wgt = 1) %>% 
  pollster::crosstab(num_staffers,
           gender_bin,
           weight = wgt, 
           pct_type = "column", 
           format = "wide") 
...
rm(by_demo, all)
#%>% 
  # add n-size to column name 
 # rename_with(.fn = ~ paste0(names(by_demo), "\n(n=", by_demo[which(num_staffers == "n"),], ")")) 
# %>% 
...
#     align = c('l', 'r', 'r'))
 ...
 # filter( ! str_detect(num_staffers, "n") )

#%>% 


# %>% 
#   unite(test, y, m, d, sep = "-")

# %>% 
#  rename_with( ~ paste() tolower(gsub(".", "_", .x, fixed = TRUE))) %>%
#  rename_with(.fn = ~ paste0(.x, "(n = ", n, ")"))
...
```{r}
all <- df %>%
  select(num_staffers, race_bin) %>%
  filter(!is.na(num_staffers), !is.na(race_bin)) %>%
  mutate(race_bin = "All")
...
```{r}
all <- df %>%
  select(num_staffers, race_bin2) %>%
  filter(!is.na(num_staffers), !is.na(race_bin2)) %>%
  mutate(race_bin2 = "All")
...
all <- df %>%
  ...
  select(num_staffers, gender_race) %>% 
  mutate(gender_race = "All")

...
To Dos: 
-=
- do current subsetting 

- skills_comfort.resp.web_dev needs to be updated
- people with outliers. should keep in universe or make that value NULL? 
- response crosstabs (management x gender)
- to the mutliselect variables, pick 

* Figure out alt ways of naming the gender bin values

Functions: 
* Create salary crosstab function (need to convert Mean to currency)
* tt function: Remove lines between variables
* histogram function: add mean line 
* For multiple response question function, add N for each response 
* Create negotiation x demographic crosstab function 
* Create binar

- better logic for salary crosstabs median
...
# Salary
## If someone select 0 as a salary, code as NA. 
...
### Age 
**What is your year (YYYY) of birth?**
```{r}
...
```{r, message = FALSE, echo = FALSE }
tt(df$job_team_function, order_prop = T) 
```
...
### By Gender 
ToDO: deal with this error message : Vectorizing 'formattable' elements may not preserve their attributes
...
Data cleaning code needs to be updated. 
...
### Management
Need to add in none of the above responses. 
...
```{r, message = FALSE, echo = FALSE }
tt(df$prof_dev_available.resp.tution_reimbursement)
```
...
to do: add logic to only count responses where the Available was select 
```{r, message = FALSE, echo = FALSE }
mult_resp(df, "prof_dev_used", order_prop = T)
```

# Covid 
Not sure if it should be its own section or nested with something else
...
```{r, message = FALSE, echo = FALSE }
df1 <- df %>%
  select(gender, compensation, compensation_bin, compensation_bin_numeric) %>%
  # remove anyone that is missing gender or salary data 
   filter(!is.na(gender), !is.na(compensation), !is.na(compensation_bin)) %>%
...
summarise(
  ...
  select(gender, n, mean, median = compensation_bin) %>%
  # clean-up output

...
#library(tidyverse)
#devtools::install_github("tidyverse/dplyr") need to get dev verison to use across
```
...
data <- googlesheets4::read_sheet(url, sheet = '2/22: Data')
...
# Mahelet: Commented out code isnt working for, so using an alternative 
# df <- data %>% 
#   select(
#     all_of(keep_cols)
#   )

df <- data[!is.na(names(data))]
```
...
```{r}
# Mahelet: Need to remove the fields 'role_next_year' and 'country' b/c there are two fields with the same name 
...
```{r}
googlesheets4::write_sheet(df, url, sheet = 'data_clean_20200222')
```

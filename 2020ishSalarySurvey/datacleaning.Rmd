...
df <- df %>%
  rename(
    ...
    "projects_done_management.resp.none" = 72
  )
...
# tmp <- df %>% filter(eligible == TRUE)
# table(tmp$data_40_org, tmp$data_40_role, tmp$data_40_team)
...
df <- df %>% mutate(
  ...
  gender_bin2 = case_when(
    gender %in% c("Cis man", "Trans man") ~ "Man", 
    gender %in% c("Cis woman", "Trans woman") ~ "Woman", 
    gender %in% c(
      "Different gender identity, prefer to specify",
      "Gender non-binary or non-conforming") ~ "Nonbinary",
    gender == "Prefer not to answer" ~ "Prefer not to answer"),
...
df <- df %>% mutate(
  ...
  race_bin = case_when(
    n_resp_selected  > 1 |
    race.resp.other_specify_freetext == "Mixed, unknown origin" |
    race.resp.other_specify_freetext == "Non White European descent."
    ~ 'Multi-Racial', 
    ...
    race.resp.prefer_not_to_answer == TRUE ~ "Prefer not to answer", 
    race.resp.other_specify == "Other, prefer to specify" & 
      n_resp_selected  == 0 ~ "Other", 
    n_resp_selected  == 0 ~ 'Missing'
  ),
  race_bin2 = case_when(
    race_bin != 'White' &
      race_bin !=  'Prefer not to answer' &
      race_bin != 'Missing' ~ 'POC/Other', 
    TRUE ~ race_bin
    )
  )
...
```{r}
df <- df %>% mutate(
  gender_race = paste(race_bin2, gender_bin, sep = " | "), 
  gender_cis_race = paste(race_bin2, gender_cis, sep = " | "), 
  gender2_race = paste(race_bin2, gender_bin2, sep = " | ")
)
```
...
# Remove duplicates 
```{r}
table(!duplicated(df$response_id))

df <- df %>% distinct(response_id, .keep_all = TRUE)
...
# sum cols to get total years worked
df$years_worked_total <- df$years_worked_data + 
  df$years_worked_political + 
  df$years_worked_political_data

# check to see if all three columns have the same non-NA value 
df <- df %>% mutate(
  work_hist_duped = ifelse(
    any(is.na(c_across(cols = cols))), 0, 
    length(unique(c_across(cols = cols))))
)

# check to see whether some one has "worked" more years that physically possible
df$age_minus_work <- df$age - df$years_worked_total
```

Graph data to visually check for outliers 

```{r}
ggplot(df) +
  aes(x = "", y = years_worked_total) +
  geom_boxplot(fill = "#0c4c8a") +
  theme_minimal()

ggplot(df) +
  aes(x = years_worked_total) +
  geom_histogram(fill = "#0c4c8a") +
  theme_minimal()

ggplot(df) +
  aes(x = "", y = years_worked_political_data) +
  geom_boxplot(fill = "#0c4c8a") +
  theme_minimal()

ggplot(df) +
  aes(x = "", y = years_worked_political) +
  geom_boxplot(fill = "#0c4c8a") +
  theme_minimal()

ggplot(df) +
  aes(x = "", y = years_worked_data) +
  geom_boxplot(fill = "#0c4c8a") +
  theme_minimal()

# print outliers 
sort(boxplot.stats(df$years_worked_total, coef = 2)$out)
sort(boxplot.stats(df$years_worked_political_data, coef = 2)$out)
sort(boxplot.stats(df$years_worked_political, coef = 2)$out)
sort(boxplot.stats(df$years_worked_data, coef = 2)$out)
```

```{r}
# Mark a whole row as NA if: 
# All three values are the same 
# Any of three values is an outlier with coef = 2x IQR
# age_minus_work < 18
# years_worked_political_data = years_worked_political, and both are above 3

df <- df %>% mutate(
  work_hist_eligible = 
    # find all the non-eligible rows, then flip so eligible = TRUE
    !(
      # if any of these conditions are met, the row should not be used
      any(is.na(c_across(cols = cols))) |
        # all columns have the same value 
        work_hist_duped == 1 | 
        # more years worked than is reasonable 
        age_minus_work < 18 | 
        # years worked is an extreme outlier
        years_worked_total %in% boxplot.stats(df$years_worked_total, coef = 2)$out | 
        years_worked_data %in% boxplot.stats(df$years_worked_data, coef = 3)$out | 
        years_worked_political_data %in% boxplot.stats(df$years_worked_political_data, coef = 3)$out | 
        years_worked_political %in% boxplot.stats(df$years_worked_political, coef = 3)$out | 
        # years worked in politics and in data listed as the same not-small number
        (identical(years_worked_political_data, years_worked_political) &
           years_worked_political_data > 4
        ) 
    )
)

# visually check data to see it seems reasonable 
outliers_all <- df %>% 
  filter(work_hist_eligible == FALSE) %>% 
  select(
    response_id, 
    status,
    starts_with("years_worked"), 
    work_hist_duped, 
    age_minus_work, 
    compensation
  )
...
Did people only use professional development that they said they had access to? 

```{r}
# Create Helper Objects
pd_df <- df %>% 
  select(
    starts_with("prof_dev"), 
    - prof_dev_yesno, 
    - ends_with("not_used")
  )
pd_types <- stringr::str_split_fixed(colnames(pd_df), ".resp.", 2)[, 2] %>% 
  unique()
...
```{r}
# Quick Checks 
# First Q validation worked correctly, only people who have PD overall used it
tmp2 <- df %>% 
  group_by(prof_dev_yesno) %>%
  summarise( 
    n = n(), 
    across(starts_with("prof_dev"), ~ sum(., na.rm = TRUE))
  )

# overall, there is no category where more people used PD than said they had it
tmp3 <- df %>% 
  summarise(
    across(starts_with("prof_dev_available"), ~sum(.x, na.rm = TRUE))
  ) %>%
  tidyr::pivot_longer(cols = everything()) %>%
  transmute(
    name = stringr::str_split_fixed(name, ".resp.", 2)[, 2], 
    n_available = value
  )

tmp4 <- df %>% 
  summarise(
    across(starts_with("prof_dev_used"), ~ sum(., na.rm = TRUE))) %>%
  tidyr::pivot_longer(cols = everything()) %>%
  transmute(
    name = stringr::str_split_fixed(name, ".resp.", 2)[, 2], 
    n_used = value
  ) %>%
  full_join(tmp3, by = "name")
```

```{r}
recode_pd <- function(dat, pd_type){
  
  available <- paste0("prof_dev_available.resp.", {{pd_type}})
  used <- paste0("prof_dev_used.resp.", {{pd_type}})
  
  errs <- dat %>% filter(
    is.na(.data[[available]]), 
    .data[[used]] == TRUE
  ) %>% nrow()
  
  print(paste("There are", errs, "errors for", pd_type, ". Cleaning Now!"))

  dat <- dat %>% mutate(
    tmp = case_when(
      is.na(dat[[available]]) ~ NA,
      dat[[used]] == TRUE ~ TRUE,
      is.na(dat[[used]]) ~ FALSE
  ))

  return(dat$tmp)
}

for(i in seq_along(pd_types)) {
  
  used <- paste0("prof_dev_used.resp.", pd_types[i])
  df[[used]] <- recode_pd(df, pd_types[i]) 
}
```
...
file <- googledrive::drive_find("survey_responses_clean.csv", n_max = 1)

googledrive::drive_update(
  googledrive::as_id(file), 
  here::here("data", "survey_responses.csv"),
  )
```
